...
cag_dmrt1F_DF <-as.data.frame(cag_dmrt1F_res[!is.na(cag_dmrt1F_res$stat),])
blah<-select(org.Mm.eg.db,keys=rownames(cag_dmrt1F_DF),keytype="ENSEMBL",
...
cag_dmrt1F_DF_subset<-cag_dmrt1F_DF[which(cag_dmrt1F_DF$padj < 0.05 &
                                            abs(cag_dmrt1F_DF$log2FoldChange) > 2),]
nrow(subset(cag_dmrt1F_DF_subset,log2FoldChange >2))
nrow(subset(cag_dmrt1F_DF_subset,log2FoldChange <2))
```
...
#gsea$avgLFC<-gsea$avgLFC/mean(gsea$avgLFC)
sum(gsea$numGenes)
...
chrdata<-chrdata[,c("total_balls","drawn_balls","pval","gsea_avgLFC",
                    "gsea_strength","gsea_pvalue","gsea_padj")]
colnames(chrdata)<-c("genes_on_chr","enriched_genes_on_chr","hypergeometeric_padj",
                     "gsea_avgLFC","gsea_strength","gsea_pvalue","gsea_padj")
...
### Supplemental Figure S2B - PieChart

```{r PieChart,warning=FALSE,fig.height=8}
#pie chart
...
pie <- ggplot(temp,aes(x="", y=Number_of_Enriched_Genes, fill = Chromosome )) +
       geom_bar(width = 1,stat="identity",colour="black") + coord_polar(theta="y") +
       geom_text(aes(x=1.7,y = 122, label = "chrX")) +  theme_bw()
pie
```
<div class="pagebreak"></div>

### Supplemental Figure S2C - GSEA
```{r plotGSEA,warning=FALSE}
#Plot AvgLFC across chromosomes
temp<-data.frame(avgLFC=chrdata$gsea_avgLFC,chromosome=rev(rownames(chrdata)))
levels(temp$chromosome)<-rev(rownames(chrdata))
temp<-melt(temp,value.name="avgLFC")
#plot gsea_avgLFC
q<-ggplot(temp,aes(x=chromosome,y=avgLFC)) +geom_bar(stat = "identity") + theme_bw() +
...
#extract fpkm values
rd<-as.data.frame(fpkm(cag_dds))

#subset for differentially expressed genes
rd<-rd[rownames(rd) %in% rownames(cag_dmrt1F_DF_subset),]
rd<-rd[,c(3,4,1,2,5,6)]
...
blah<-select(org.Mm.eg.db,keys=rownames(WT),keytype="ENSEMBL",column="SYMBOL")
WT_res$symbol<-blah[match(rownames(WT_res),blah$ENSEMBL),"SYMBOL"]
WT_res[grep("Foxl2",WT_res$symbol),]
WT_res[grep("Sox9",WT_res$symbol),]
WT_res[grep("Cyp19a1",WT_res$symbol),]
...
```{r EDA} 
load("ctv_single_mm9_100214_exonsByGene.rdata")
load("ctv_single_mm9_100214_exonsByGene_miseq.rdata")
...
```{r SingleCellpData}
qc<-read.csv("zarkower_project_021_cDNA_QC.csv",stringsAsFactors=F,header=F)
qc<-qc[qc$V7,]
...
blah<-select(org.Mm.eg.db,keys=rownames(fd),keytype="ENSEMBL",column="SYMBOL")
...
#Gene Ensemble IDs
test_genes <- select(org.Mm.eg.db, keys=test_genes, columns="ENSEMBL",keytype="ENTREZID")$ENSEMBL
...
##Create Figure 3C & Supplmental Table 1c
