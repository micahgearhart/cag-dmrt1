...
```{r S2PieChart,fig.height=8,fig.width=6}
#Set up Color Palette
colors <- brewer.pal(4, "YlOrRd")
pal <- colorRampPalette(colors)
...
for (gene in temp) {
    a[[gene]]<-plot_symbol_in_pseudotime(gene)+ theme(legend.position="none")
if  (paste("^",gene,"$") in tT$Gene.symbol) {
...
#sanity Check
temp<-c("Serpine2","Mmp2","Cst9","Col1a1","Aard","Rhox8")
ediff[ediff$symbol %in% temp,]

eDM<-ediff[abs(ediff$ce) > 1 & ediff$EmbryonicallyDimorphic=="yes",]
write.csv(eDM,file="embryonically_dimorphic39.csv",quote=F)
...
middle<-rownames(temp[temp$Pseudotime > q1 & temp$Pseudotime < q2,])
ends<-rownames(temp[temp$Pseudotime < q1 | temp$Pseudotime > q2,])
cd_groups<-factor(c(rep("middle",length(middle)),rep("ends",length(ends))),levels=c("middle","ends"))
names(cd_groups)<-c(middle,ends)
table(cd_groups)
...
ediff_subset<-ediff[abs(ediff$cZ) > 1 & abs(ediff$OvaryLog2FC) < 100 & ediff$EmbryonicallyDimorphic =="yes",]
ediff_subset<-ediff[abs(ediff$cZ) > 0.5,]
